...
void Main()
{
	var provider = new ServiceCollection()
		...
		.AddTransient(typeof(IFoobar<,>), typeof(Foobar<,>)).BuildServiceProvider();

	var footBar = (Foobar<IFoo, IBar>)provider.GetService<IFoobar<IFoo, IBar>>();
	Debug.Assert(footBar?.Bar is Bar);
	Debug.Assert(footBar?.Foo is Foo);
}
